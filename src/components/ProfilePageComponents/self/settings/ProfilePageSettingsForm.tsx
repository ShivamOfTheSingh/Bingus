...
export default function ProfilePageSettingsForm({ profile, settings, className }: ProfilePageSettingsFormProps) {
    ...
    const [profilePublic, setProfilePublic] = useState<boolean>(settings.profilePublic);
    ...
    const [profilePictureSource, setProfilePictureSource] = useState<string>(profile.profilePicture);
    ...
    async function onSubmit() {
        ...
        if (!validateFields.success) {
            ...
            else if (profileResponse.status === 401 || settingsResponse.status === 401) {
                throw new ApiError("Session is inactive. Please log in again.", 401);
            }
            else {
                throw new ApiError("What the Bingus? An unexpected error occured.", 500);
            }
        }
    }
    ...
    return (
        <Form action={onSubmit} className="flex flex-col justify-center">
            <Form.Group controlId="profilePicture">
                <Image src={profilePictureSource || tempProfilePicture} alt="profile-picture" width={200} height={200} className="rounded-full" />
                <Form.Control type="file" onChange={addFile} disabled={pending} />
                {validateErrors.profilePicture && <Form.Label className="text-red-600">{validateErrors.profilePicture}</Form.Label>}
                {profilePictureFile &&
            ...
            <Form.Group controlId="about">
                ...
                <Form.Control
                    ...
                    placeholder="About"
                    value={about}
                    onChange={(e) => { setAbout(e.target.value); setValidateErrors({ ...validateErrors, about: null }) }}
                    disabled={pending}
                />
                {validateErrors.about && <Form.Label className="text-red-600">{validateErrors.about}</Form.Label>}
            </Form.Group>
            <Form.Group controlId="showName">
                <Form.Check
                    type="checkbox"
                    label="Show Full Name"
                    onChange={(e) => { setShowName(!showName); setValidateErrors({ ...validateErrors, showName: null }); }}
                    disabled={pending}
                    checked={showName}
                />
                {validateErrors.showName && <Form.Label className="text-red-600">{validateErrors.showName}</Form.Label>}
            </Form.Group>
            <Form.Group controlId="profilePublic">
                <Form.Check
                    type="checkbox"
                    label="Profile Public"
                    onChange={(e) => { setProfilePublic(!profilePublic); setValidateErrors({ ...validateErrors, profilePublic: null }); }}
                    disabled={pending}
                    checked={profilePublic}
                />
                {validateErrors.profilePublic && <Form.Label className="text-red-600">{validateErrors.profilePublic}</Form.Label>}
            </Form.Group>
            <Form.Group controlId="submit">
                <Button variant="primary" type="submit" disabled={pending}>
                    {pending ? <div className="flex gap-2 items-center"><Spinner size="sm" animation="border" />Submitting...</div> : "Submit"}
                </Button>
            </Form.Group>
            {success && (
                <div
