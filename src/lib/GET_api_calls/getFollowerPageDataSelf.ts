...
export async function getFollowerPageData(userId: number): Promise<ReturnData> {
    try {
        ...
        for (let i = 0; i < usersFiltered.length; i++) {
            const followingResponse = await fetch(`https://production.d3drl1bcjmxovs.amplifyapp.com/api/crud/followings/pair?selfId=${usersFiltered[i].userId}&otherId=${userId}`);
            if (followingResponse.status === 200) {
                const innerFollowingResponse = await fetch(`https://production.d3drl1bcjmxovs.amplifyapp.com/api/crud/followings/pair?selfId=${userId}&otherId=${usersFiltered[i].userId}`);
