...
export default async function getProfilePageSettingsData(userId: number): Promise<ReturnData> {
    const profileResponse = await fetch(`http://localhost:3000/api/crud/user_profile/${userId}`);
    const settingsResponse = await fetch(`http://localhost:3000/api/crud/user_settings/user/${userId}`);
