...
export default async function getProfilePageData(userId: number): Promise<ReturnData> {
    ...
    const resFollowers = await fetch(`https://production.d3drl1bcjmxovs.amplifyapp.com/api/crud/followings/numFollowers/${userId}`);
    const numFollowers = await resFollowers.json();

    const resFollowing = await fetch(`https://production.d3drl1bcjmxovs.amplifyapp.com/api/crud/followings/numFollowing/${userId}`);
    const numFollowing = await resFollowing.json();

    const resSettings = await fetch(`https://production.d3drl1bcjmxovs.amplifyapp.com/api/crud/user_settings/user/${userId}`);
